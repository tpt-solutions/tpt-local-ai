...
- [ ] Fix path traversal: sanitize `filename`/server-provided `rfilename` in `snapshot_download` (`client.rs:98`) to reject `..` segments and absolute paths before joining into `dest_dir`
- [ ] Verify resumed downloads actually get `206 Partial Content` back before appending (`client.rs:122-159`); treat a `200` response to a `Range` request as "restart from scratch", not "append"
- [ ] Add file locking (or an in-process mutex keyed by the tmp path) around the deterministic `.tmp` path (`client.rs:254-258`) to prevent concurrent-download corruption
- [ ] Fix `rename`-over-existing-file TOCTOU (`client.rs:191`) so behavior is consistent on Windows vs POSIX
- [ ] Harden `validate_repo_id` (`client.rs:247-252`) to reject `..` segments and Windows absolute paths (`C:\...`)
- [ ] Fix `cache.rs:44-49` test flakiness: don't mutate the process-global `TPT_HUB_CACHE` env var without synchronization across parallel tests
- [ ] Add auth-token support (`Authorization: Bearer <token>`, `HF_TOKEN` env var, `.with_token(...)`) for gated/private models
- [ ] Add offline mode (`HF_HUB_OFFLINE`-style: cache-only, never hit network)
- [ ] Support `HF_ENDPOINT` env var convention for mirrors
- [ ] Add retry/backoff on transient network failures
- [ ] Parallelize sibling downloads in `snapshot_download` (currently strictly sequential) with a concurrency limit knob
- [ ] Add regression tests: Range-ignored-by-server corruption, path traversal via `rfilename`, concurrent download of the same file
- [ ] Add a Windows (and ideally macOS) job to `.github/workflows/ci.yml` — currently `ubuntu-latest` only, despite Windows-sensitive rename/path logic
...
- [ ] Fix UTF-8 mangling: template text and quoted-string literals are built with `bytes[i] as char` (`parser.rs:90`, `parser.rs:437`), which mojibake-corrupts any multi-byte UTF-8 (accents, CJK, emoji); rewrite to be UTF-8-aware like `value.rs`'s JSON string parser
- [ ] Fix potential panic in `find_close` (`parser.rs:102-113`): `str` slicing (`src[i..i+close.len()]`) while byte-scanning for `}}`/`%}`/`#}` can panic with "byte index is not a char boundary" on non-ASCII template source
- [ ] Add filter (`|`) support: `tojson`, `trim`, `default`, `join`, `upper`/`lower`, `length`, `first`/`last`, `selectattr`, `map`, `reject`, `string` — `tojson` alone is used pervasively in real Llama 3.1/3.2 tool-calling templates and blocks most real-world adoption without it
- [ ] Add `is` test expressions (`is defined`, `is none`, `is string`, `is iterable`, etc.)
- [ ] Add `~` string-concatenation operator (only `+` supported today)
- [ ] Add list literal support (`[]`, `['user', 'assistant']`) — currently unparseable
- [ ] Add function-call syntax (`Expr::Call`) to unblock `raise_exception(...)`, `.items()`, `namespace()` idioms used by real Hub templates (Mistral/Zephyr role-alternation checks, Qwen-style `namespace()` state)
- [ ] Add tuple/multi-variable `for` target (`{% for k, v in x.items() %}`)
- [ ] Add non-ASCII template regression test (would have caught the mojibake bug)
- [ ] Add a `cargo-fuzz` target for the scanner/parser (handles untrusted template input by design, zero fuzz coverage today)
...
- [ ] Fix integer overflow in header-length arithmetic (`reader.rs:194-199`, `reader.rs:274`): `8 + header_len` / `8 + header_len + info.end` can overflow `usize` on a corrupted/adversarial header, silently wrapping in release builds and defeating the bounds check
- [ ] Add a cross-check that each tensor's `data_offsets` span equals `dtype.size_bytes() * numel()` before returning a `TensorView`, so callers reading `.data` directly are protected against a truncated/oversized declared region
- [ ] Add a `# Safety` doc note on the `Mmap::map` call (`reader.rs:134`) — security-relevant entry point exposed transitively to `tpt-lora-merge`
- [ ] Add `F8_E4M3`/`F8_E5M2` (fp8) dtype support (increasingly common in quantized checkpoints)
- [ ] Add `U16`/`U32`/`U64` dtype support (part of the safetensors spec)
- [ ] Add a streaming/chunked write path to `SafetensorsBuilder` — currently buffers the entire output file in a `Vec<u8>`, undermining the crate's "zero-copy" pitch on the write side
- [ ] Consider an API to patch/replace a subset of tensors in an existing file without rebuilding from scratch
- [ ] Add regression tests for corrupted/malicious headers (overflow, `start > end`, missing fields, non-object header)
- [ ] Add round-trip tests for F16, BF16, I8/I16/I32/I64, U8, BOOL (currently only F32 is tested)
- [ ] Add a `cargo-fuzz` target for header parsing (explicitly handles adversarial input per its own doc comments, zero fuzz coverage today)
- [ ] Set `overflow-checks = true` in the release profile, or otherwise guard against silent wraparound in offset arithmetic workspace-wide
...
- [ ] Address O(n²) per-word merge loop in `BpeTokenizer::tokenize_word` (`bpe.rs:63-89`) — perf cliff for pathologically long "words" (e.g. whitespace-free blobs, since `split_words` only splits on whitespace)
- [ ] Add byte-level pre-tokenization / byte-fallback so `encode` never fails on arbitrary Unicode (currently returns `UnknownToken` for any char/sub-word missing from vocab with no `<unk>`)
- [ ] Add GPT-2-style pre-tokenizer regex splitting (contractions, punctuation, digits) instead of bare whitespace splitting
- [ ] Add BERT "basic tokenize" pass for WordPiece (lowercasing, accent stripping, CJK character spacing, punctuation splitting)
- [ ] Add a loader for the modern unified `tokenizer.json` format (most current Hub repos ship this, not legacy `vocab.txt`+`merges.txt`) — likely the single highest-leverage adoption fix for this crate
- [ ] Add special-token handling (BOS/EOS/CLS/SEP auto-insertion, `added_tokens`, padding/truncation, batch encoding API)
- [ ] Add Unicode normalization (NFC/NFKC) before tokenization
- [ ] Add tests for `from_files`/`from_file` disk-loading constructors, malformed vocab/merges files, empty input, `max_input_chars_per_word` overflow behavior, non-ASCII/multi-byte input
...
- [ ] Preserve original `__metadata__` from base/LoRA safetensors files on merge (`merge.rs:59-67` currently only writes a `"merged_by"` key, discarding provenance/license/quantization metadata)
- [ ] Error (not silently copy-through) when only one of `lora_A`/`lora_B` is present for a module (`merge.rs:107-113`) — likely indicates a corrupt/mis-named adapter file
- [ ] Verify every tensor in the LoRA file was actually consumed; error/warn on unused adapter tensors instead of silently no-op'ing the whole merge (exit code 0) on a naming-convention mismatch
- [ ] Support multiple LoRA adapters in one merge (weighted sum), not just a single `--scale`
- [ ] Read `adapter_config.json` to auto-derive `alpha`/`r` instead of requiring manual `--scale`
- [ ] Support alternate LoRA naming conventions (Kohya `lora_down`/`lora_up` + per-layer `.alpha` tensors, PEFT multi-adapter `.lora_A.<adapter_name>.weight`)
- [ ] Preserve base dtype instead of always upcasting merged tensors to F32 (currently silently doubles output size for BF16/F16 bases); at minimum warn the CLI user about the size growth
- [ ] Add a `--dry-run`/`--check` mode to validate adapter/base tensor alignment before committing to a merge+write
- [ ] Add tests: shape-mismatch error path, partial-adapter-pair case, metadata preservation, multiple adapted tensors, non-2D tensor alongside a same-named adapter
